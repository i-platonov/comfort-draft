...
export default function HeatTab() {
  ...
  const totalHeatW = zones.reduce(
    (sum, zone) => sum + zoneHeatOutputW(mmToMeters(zone.spiralLengthMm), flowLpmPer100m, supplyTempC, returnTempC),
    0,
  );
  const totalFlowLpm = zones.reduce((sum, zone) => sum + zoneFlowLpm(mmToMeters(zone.spiralLengthMm), flowLpmPer100m), 0);
    ...
    <div className="side-panel-tab-content">
      ...
      <section className="panel-section zones-section">
        ...
        <div className="zone-list">
          {zones.map((zone) => {
            const flowLpm = zoneFlowLpm(mmToMeters(zone.spiralLengthMm), flowLpmPer100m);
            const heatW = zoneHeatOutputW(mmToMeters(zone.spiralLengthMm), flowLpmPer100m, supplyTempC, returnTempC);
            ...
            return (
              <div key={zone.id} className="zone-card" style={{ borderLeftColor: zone.color, cursor: 'default' }}>
                ...
                <span className="info">
                  {flowLpm.toFixed(2)} L/min · {wPerM2.toFixed(0)} W/m² · {mmToMeters(zone.spiralLengthMm).toFixed(1)}m loop
                </span>
              </div>
            );
          })}
